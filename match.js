...
<section class="how-section" id="how">
  ...
  <div class="steps-grid">
    ...
    <div class="step">
      ...
      <p>We return your top 3 neighbourhood matches with a similarity score, personality explanation, food & wine picks, and direct booking links.</p>
    </div>
  </div>
</section>
...
<section class="matches-section" id="examples">
  ...
  <div class="matches-scroll">
    ...
    <div class="match-pair">
      <div class="match-photo" style="background-image:url('https://images.unsplash.com/photo-1558618047-3c8c76ca7d13?w=600&q=80')">
    ...
    <div class="match-pair">
      <div class="match-photo" style="background-image:url('https://images.unsplash.com/photo-1529156069898-49953e39b3ac?w=600&q=80')">
        <div class="match-photo-overlay">
    ...
    <div class="match-pair">
      <div class="match-photo" style="background-image:url('https://images.unsplash.com/photo-1551862777-1c0b3d3a32b9?w=600&q=80')">
        <div class="match-photo-overlay">
    ...
    <div class="match-pair">
      <div class="match-photo" style="background-image:url('https://images.unsplash.com/photo-1493993544685-a94361dc1e1b?w=600&q=80')">
        <div class="match-photo-overlay">
    ...
    <div class="match-pair">
      <div class="match-photo" style="background-image:url('https://images.unsplash.com/photo-1583422409516-2895a77efded?w=600&q=80')">
        <div class="match-photo-overlay">
...
<section class="why-section">
  ...
  <div class="why-grid">
    ...
    <div class="why-card">
      ...
      <h3>Built by a travel insider</h3>
      <p>Founded by an airline distribution specialist with a decade inside the aviation industry. Every data point is chosen by someone who genuinely travels for a living.</p>
    </div>
...
<section class="pricing-section" id="pricing">
  ...
  <div class="pricing-grid">
    ...
    <div class="price-card">
      ...
      <a href="#newsletter" class="price-btn price-btn-outline">Get started free</a>
    </div>
    ...
    <div class="price-card price-card-featured">
      ...
      <a href="#newsletter" class="price-btn price-btn-light">Start for €49/year</a>
      <p style="font-size:11px; margin-top:10px; opacity:0.6; text-align:center;">or €5.90/month, cancel anytime</p>
    </div>
    ...
    <div class="price-card price-card-lifetime">
      ...
      <a href="#newsletter" class="price-btn price-btn-gold">Claim lifetime access</a>
...
<section class="newsletter-section" id="newsletter">
  <div class="newsletter-inner">
    ...
    <div class="newsletter-form">
      ...
      <button onclick="subscribeEmail()">Join waitlist</button>
    </div>
...
<footer>
  ...
  <ul class="footer-links">
    ...
    <li><a href="/cdn-cgi/l/email-protection#d2bab7bebebd92bfb3a6b1babfabbabdbdb6fcb1bdbf">Contact</a></li>
...
function setIntent(type) {
  ...
  // Update search section label based on intent
  const label = type === 'move'
    ? 'Find where to live — match your neighbourhood'
    : 'Find where to stay — match your neighbourhood';
  const h2 = document.querySelector('.search-inner h2');
  if (h2) h2.innerHTML = label.replace('match', '<em>match</em>');
}
...
async function runMatch() {
  ...
  try {
    const response = await fetch('/api/match', {
      ...
      body: JSON.stringify({ homeCity, homeHood, destCity })
    });
    ...
    renderResults(matches, homeHood, homeCity, destCity);

  } catch (err) {
    showError('Something went wrong finding your match. Please try again in a moment.');
    console.error(err);
  } finally {
    btn.disabled = false;
...
function renderResults(matches, homeHood, homeCity, destCity) {
  const results = document.getElementById('resultsArea');

  const header = `<div style="margin-bottom:24px;">
    <p style="font-size:13px; color:rgba(248,241,231,0.5); letter-spacing:0.8px; text-transform:uppercase; font-weight:600; margin-bottom:8px;">✦ Your matches</p>
    <p style="font-size:18px; color:var(--cream); font-family:'Cormorant Garamond',serif; font-weight:600;">
      If you love <em style="color:var(--terracotta-light)">${homeHood}</em> in ${homeCity}, here's where you'll feel at home in ${destCity}:
    </p>
  </div>`;
  ...
  const cards = matches.map((m, i) => `
    <div class="result-card" style="animation-delay:${i * 0.1}s">
      <div class="result-header">
        <div>
          <div class="result-city">${m.city}</div>
      ...
      <div class="result-tags">
        ${(m.vibes || []).map(v => `<span class="result-tag">${v}</span>`).join('')}
      </div>
      <div class="result-extras">
        <div class="result-extra">
          <div class="result-extra-label">🍽️ Food pick</div>
          <div class="result-extra-val">${m.foodPick || '—'}</div>
        </div>
        <div class="result-extra">
          <div class="result-extra-label">🍷 Wine pick</div>
          <div class="result-extra-val">${m.winePick || '—'}</div>
        </div>
        <div class="result-extra">
          <div class="result-extra-label">💶 Cost level</div>
          <div class="result-extra-val">${m.costLevel || '—'}</div>
        </div>
      </div>
      <p style="font-size:12px; color:rgba(248,241,231,0.4); margin-top:12px;">🎯 Best for: ${m.bestFor || '—'}</p>
      <a href="https://www.booking.com/searchresults.html?ss=${encodeURIComponent(m.name + ', ' + destCity)}&affiliate_id=YOUR_ID" target="_blank" class="booking-btn">
        🏨 Find accommodation in ${m.name} →
      </a>
    </div>
  `).join('');
  ...
  results.innerHTML = header + cards + shareText;
  results.classList.add('visible');
  results.scrollIntoView({ behavior: 'smooth', block: 'start' });
}

function shareMatch(fromHood, fromCity, toHood, toCity) {
  const text = `If you love ${fromHood} in ${fromCity}, you'll love ${toHood} in ${toCity} — matched by @MatchMyHood 🏘️✨ #MatchMyHood #travel #neighbourhood`;
  if (navigator.share) {
